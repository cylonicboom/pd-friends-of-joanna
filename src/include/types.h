...
struct g_vars {
  // lv values pause when the game is paused, and reset when a new level is
  // loaded diff values run all the time names ending in f are the same as their
  // non-f counterparts, but as a float names ending in freal are the same as f
  // but multiplied by 1.2 on PAL names ending in 60 mean their units are 60ths
  // of a second names ending in 240 mean their units are 240ths of a second
  // names ending in t mean their units are CPU ticks/cycles

  /*0x000*/ s32 diffframe60;  // time between current frame start and previous
                              // frame start in 60ths of a second
  /*0x004*/ f32 diffframe60f; // always .0 decimal
  /*0x008*/ s32 lvframe60; // incrementing 60ths, but it doesn't apply remainder
                           // so it drops behind lvframe240 / 4
  ...
  /*0x01c*/ s32 diffframet; // previous frame's duration in ticks, including any
                            // wait time after it finished
  /*0x020*/ s32 lostframetime60t;
  /*0x024*/ s32 lostframetime240t;
  /*0x028*/ u32 mininc60; // minimum amount of time between frames (always set
                          // to 1, ie. max 60fps)
  ...
  /*0x038*/ s32 lvupdate60;  // update multiplier, factoring in remainder from
                             // previous frame
  /*0x03c*/ s32 lvupdate240rem; // update multiplier's remainder for next
                                // frame's lvupdate60 (0 - 3)
  /*0x040*/ s32 diffframe240;   // time between current frame start and previous
                                // frame start in 240ths of a second
  ...
  /*0x054*/ s32
      prevframestart240; // previous frame's start time in 240ths (incrementing)
  /*0x058*/ s32
      thisframestart240; // current frame's start time in 240ths (incrementing)
  ...
  /*0x290*/ s32 currentplayerindex; // 0-3 - but 2 or 3 probably only used in
                                    // combat simulator
  ...
  /*0x2a4*/ struct player
      *coop; // Co-op buddy when controlled by human;  4-player coopanti: used
             // like currentplayer, but for coop
  /*ext*/ struct player
      *coopplayers[MAX_PLAYERS]; // Co-op buddies when controlled by humans
  /*ext*/ struct player
      *allyplayers[MAX_PLAYERS];    // Bond + Co-op. Bond is always 0.
  /*ext*/ s32 currentantiplayernum; // 0-3 - controller numbers I think
  /*0x2a8*/ struct player *anti;    // Counter-op; 4-player coopanti: used like
                                    // currentplayer, but for anti
  ...
  /*0x314*/ u32 mplayerisrunning; // combat sim with any number of players, coop
                                  // with human buddy, and counter-op
  ...
  /*0x354*/ struct prop *pausedprops; // head of a doubly linked list, prev
                                      // pointer points to activepropstail
...
#ifdef AVOID_UB // there will be an OOB access otherwise; this does fuck up the
                // offsets here though
  ...
  /*0x4d2*/ s8 autocutnum; // cutscene scene number (0, 1 or 2), set to -1 while
                           // loading cutscene
  /*0x4d3*/ s8 autocutplaying; // true if playing a cutscene via the menu, false
  ...
  // 000f = Counter: if 10-15 then ticks down 1 per 7 frames, if under 10 then 1
  // per frame. 00f0 = One bit per pak. These paks are checked and ticked. 0f00
  // = One bit per pak. These paks are checked but not ticked.
...
struct anim {
  ...
  /*0x6c*/ u32 unk6c;
  /*0x70*/ bool (*unk70)(struct model *model, struct coord *arg1,
                         struct coord *arg2, f32 *ground);
  /*0x74*/ f32 playspeed;
  /*0x78*/ f32 newplay;
  /*0x7c*/ f32 oldplay;
  /*0x80*/ f32 timeplay;
...
struct waypoint {
  s32 padnum;
  s32 *neighbours; // most significant two bits are booleans, remaining bits are
                   // waypoint index
  s32 groupnum;
  s32 step;
};
...
struct aibot {
  ...
  /*0x0b0*/ f32
      lookangle; // likely to be the turn angle to 360 degrees, in radians
  /*0x0b4*/ f32 moveratex;
  /*0x0b8*/ f32 moveratey;
  ...
  /*0x11c*/ s32 shootdelaytimer60; // ticks up when target onscreen, down when
                                   // offscreen, always >= 0
...
struct geotilef {
  ...
  union {
    ...
    // properties (eg. cdCollectGeoForCylFromList). @TODO: Rematch them using
    // the arrays.
    struct {
...
struct act_die {
  ...
  /*0x40*/ struct coord extraspeed; // for changing position while airborne (eg.
                                    // when shot by magnum)
...
struct act_attack {
  ...
  /*0x37*/ s8 dooneburst;
  /*0x38*/ s8
      firegun[2]; // whether this gun is going to attempt to fire in this anim
  /*0x3a*/ s8 everytick[2]; // whether gun's fire rate is high enough to fire on
                            // every tick
...
struct waydata {
  ...
  /*0x10*/ struct coord
      obstacleleft; // world coord of left (from chr's perspective) edge of obj
  /*0x1c*/ struct coord obstacleright; // world coord of right (from chr's
                                       // perspective) edge of obj
  /*0x28*/ s32 age;
  /*0x2c*/ struct coord
      aimposobj; // left or right edge + chr's width, or aimpos if no obj
...
struct chrdata {
  ...
  /*ext*/ s32
      coopplayernum; // used to lock onto specific coop partner in team missions
};
...
struct defaultobj {
  ...
  union {
    ...
    /*0x44*/ struct geocyl *unkgeo; // temporary, to indicate that I don't know
                                    // which geo pointer is being used
  };
  union {
...
#ifndef PLATFORM_N64
  // When > 0, overrides the default pickup ammo quantity (used for
  // player-dropped guns).
  s16 ammo_qty;
#endif
};
...
struct eyespy {
  ...
  /*0x58*/ f32 speed;
  /*0x5c*/ f32 oldground;
  /*0x60*/ f32 height; // height above ground - 30 to 160 on G5 and CI training,
                       // 80 to 160 elsewhere
  /*0x64*/ f32 gravity;
...
struct hand {
  ...
  /*0x0848*/ f32 slidetrans; // 0 at rest, positive when back (struct
                             // weaponfunc_shoot.slidemax)
  /*0x084c*/ bool slideinc;  // true when moving back, false when moving forward
                             // or not moving
...
struct gunctrl {
  /*0x1580*/ s8 weaponnum;
  /*0x1581*/ s8 prevweaponnum; // previously drawn weapon, switched to when
                               // throwing Dragon/Laptop or when ammo depleted
...
struct player {
  ...
  /*0x0004*/ struct coord
      memcampos; // Room that the camera is in (differs from the player's room
                 // during cutscenes and Slayer rocket)
  ...
  // These crouch fields are related to recovering after a fall - not actual
  // crouching
  /*0x0088*/ f32 sumcrouch;
  /*0x008c*/ f32
      crouchheight; // Negative, is Y offset to regular standing height
  /*0x0090*/ s32 crouchtime240; // Set to 60 when landing, counts down
  /*0x0094*/ f32 crouchfall; // -90 when slowing the descent, increments back to
                             // 0 while returning to stand
  ...
  /*0x0174*/ f32 speedboost; // speed multiplier - ranges from 1 to 1.25 - kicks
                             // in after 3 seconds of full speed
  /*0x0178*/ s32
      speedmaxtime60; // amount of time player has held full forward speed
  ...
  /*0x16f4*/ f32 gunzoomfovs[3]; // saved zoom levels/fovs for sniper, farsight
                                 // and horizon scanner
  ...
  /*0x1924*/ s32
      healthdamagetype; // 0-7, decreases based on amount of damage taken
  ...
  /*0x19b8*/ s32 crouchoffsetrealsmall; // 0 = standing, -90 = squatting, can be
                                        // between during transition
  ...
  /*0x1bf8*/ s32
      bondcurexplode; // Increases by 1 on each tick even when not exploding
  ...
  /*ext*/ s8
      livesremaining; // Current lives left for this player (-1 = unlimited)
  ...
  /*0x1c6c*/ u32 unk1c6c;
  /*0x1c70*/ s16
      altdowntime; // for alt-modes, used like invdowntime and amdowntime
  /*0x1c72*/ s16
      amdowntime; // for alt-modes, used like invdowntime and amdowntime
#ifndef PLATFORM_N64
...
struct weaponfunc_throw {
  ...
  /*0x18*/ s16 activatetime60; // time until proxies become active, or timed
                               // mine/grenade explodes
...
struct menuitem {
  ...
  union {
    uintptr_t (*handler)(s32 operation, struct menuitem *item,
                         union handlerdata *data);
    void (*handlervoid)(s32 operation, struct menuitem *item,
                        union handlerdata *data);
  };
};
...
struct menudialogdef {
  ...
  s32 (*handler)(s32 operation, struct menudialogdef *dialogdef,
                 union handlerdata *data);
  u32 flags;
...
struct room {
  /*0x00*/ u16 flags;
  /*0x02*/ s16 loaded240; // 0 when unloaded, 1 when visible, ticks up to 120
                          // when recently visible
...
struct menudialog {
  ...
  /*0x40*/ f32 transitionfrac; // >= 0 means transitioning from one dialog type
                               // to another
  ...
  /*0x60*/ u8 state;
  /*0x64*/ s32
      scroll; // scroll related, 0 when at top, negative when scrolled down
...
struct gamefile {
  /*0x00*/ char name[11];
  /*0x0b*/ u8 thumbnail
      : 5; // stage index of the image to show on file select screen
...
struct mpchrconfig {
  ...
  /*0x18*/ u16
      unk18; // managed by func0f179b68, possibly health or shield (unused)?
  /*0x1a*/ u16
      unk1a; // managed by func0f179c14, possibly health or shield (unused)?
  /*0x1c*/ u16 unk1c; // managed by func0f179cc0, it's the unused speed modifier
                      // for the character. the wiring in bondwalk is there but
                      // the dialog was disabled
...
struct missionconfig {
  ...
  u8 isteam;
  /*ext*/ s8 lives; // -1 to 100: lives mode (-1=unlimited, 0=standard, 1+=that
                    // many lives)
};
...
struct scenariodata_htm {
  ...
  /*0x800ac18c*/ struct htmterminal
      terminals[7]; // only the first element is used
...
struct activemenu {
  ...
  /*0x14*/ s32 returntimer; // time before selection returns to middle after
                            // releasing control stick
  /*0x18*/ f32 alphafrac;
  ...
  /*0x30*/ u8 togglefunc;
  /*0x31*/ u8 numitems; // number of items in player's inventory; can be higher
                        // than the number of AM slots
  /*0x32*/ u8 allbots;  // when player holds R on the bot command screen
  /*0x33*/ u8
      prevallbots; // used when opening "Pick Target" menu for attack command
...
struct smoke {
  ...
  // If source is set, option 0/1 determines whether the source is a prop/pad
  // effect If source is null, option 0/1 is the handnum (right/left)
  /*0x006*/ u16 option : 1;
...
struct frtarget {
  ...
  /*0x32*/ u8
      donestopsound; // 1 if the clank sound has played when stopping travelling
  /*0x33*/ u8 travelling;
  /*0x34*/ s8 frpadnum;
...
struct weathercfg {
  ...
  f32 zmax;
  RoomNum
      skiprooms[WEATHERCFG_MAX_SKIPROOMS]; // if flags has WEATHERFLAG_INCLUDE,
                                           // rooms that have weather, else
                                           // rooms that don't
  f32 windanglerad;                        // wind fields only used if flags has
                                           // WEATHERFLAG_FORCE_WINDDIR
  f32 windspeedx;
  f32 windspeedz;
};
...
struct menuinputs {
  /*0x00*/ s8 leftright; // Both control stick and C/D buttons - set on initial
                         // press and on key repeat intervals
  ...
  /*0x08*/ s8 leftrightheld; // Same as leftright, but is also set between
                             // repeat intervals
