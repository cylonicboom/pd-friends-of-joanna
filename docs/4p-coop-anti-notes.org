 * AIcmds using =CHR_COOP= / =CHR_ANTI= / =CHR_P1P2=


  These will need to be updated to refer to all valid coop / anti / p1p2 players

** DONE anti compatibility: look at ifChrHasObject to verify that it's only checking coop characters
   Uses playerpool, which can handle anti
   #+begin_src c
/**
 * @cmd 005d
 */
...
** DONE =void cctvTick(struct prop *camprop)=

   vanilla: This cylces between coop and bond alternating frames.
   4playercoop: This cycles between all players, including bond and coop. The ticks needed to trip the alarm is divided by the number of bond + coop players.

**  WONTDO src/include/commands.h|688 col 1| #define if_chr_in_onscreen_room(chr, label) \
**  WONTDO src/include/commands.h|830 col 1| #define if_chr_distance_to_pad_lt(chr, distance, pad, label) \
**  WONTDO src/include/commands.h|841 col 1| #define if_chr_distance_to_pad_gt(chr, distance, pad, label) \
**  WONTDO src/include/commands.h|902 col 1| #define if_chr_in_room(chr, type, room_or_pad, label) \
**  DONE src/include/commands.h|921 col 1| #define if_chr_has_object(chr, object, label) \
   Coop support only.
**  DONE src/include/commands.h|948 col 1| #define if_chr_weapon_equipped(chr, weapon, label) \
   Coop support only.
  #+begin_src
...
if_chr_weapon_equipped(CHR_P1P2, WEAPON_UNARMED, /*goto*/ 0x03)
  #+end_src
**  WONTDO src/include/commands.h|980 col 1| #define if_chr_activated_object(chr, object, label) \
   NTD? flags get set on the object during the tick when inputs are checked
**  WONTDO src/include/commands.h|1249 col 1| #define if_chr_health_gt(chr, health, label) \
   NTD? Unused.
**  WONTDO src/include/commands.h|1258 col 1| #define if_chr_health_lt(chr, health, label) \
   NTD? Unused.
**  WONTDO src/include/commands.h|1375 col 1| #define if_chr_alertness_lt(value, chr, label) \
   NTD? NPC related?
...
**  TODO src/include/commands.h|1631 col 1| #define if_chr_has_chrflag(chr, chrflag3, label) \
   Not yet implemented
**  WONTDO src/include/commands.h|2459 col 1| #define if_chr_target_eq(chr1, chr2, anytarget, label) \
   NTD? Only used to compare =CHR_SELF= to =CHR_P1P2= in a loop
**  WONTDO src/include/commands.h|2530 col 1| #define if_chr_shield_lt(chr, value, label) \
   NTD? Only used for NPCs and =CHR_SELF=
**  WONTDO src/include/commands.h|2539 col 1| #define if_chr_shield_gt(chr, u1, label) \
   NTD. Unused.
**  TODO src/include/commands.h|2666 col 1| #define if_chr_has_hiddenflag(chr, chrflag2, label) \
   NTD? Not implemented currently. Most scripts are written to loop in a way that works with the player cycling.
**  TODO src/include/commands.h|2677 col 1| #define if_chr_is_human(chr, label) \
   NTD? Not needed until AIO / PD+ support is added
**  TODO src/include/commands.h|2685 col 1| #define if_chr_is_skedar(chr, label) \
   NTD? Not needed until AIO / PD+ support is added
**  WONTDO src/include/commands.h|2989 col 1| #define if_chr_in_squadron_doing_action(action, label) \
   NPC AIcmd
**  WONTDO src/include/commands.h|3180 col 1| #define if_chr_listening(chr, listenvalue, checktype, label) \
   NTD. Unused.
**  WONTDO src/include/commands.h|3248 col 1| #define if_chr_injured_target(chr, label) \
   NTD. Reads flag set during the tick.
**  WONTDO src/include/commands.h|3273 col 1| #define if_chr_shield_damaged(chr, label) \
   NTD. Used for NPCs.
**  WONTDO src/include/commands.h|3278 col 1| #define if_chr_idle_action_eq(action, label) \
   NTD. Used for NPCs.
**  TODO src/include/commands.h|3289 col 1| #define if_chr_y(chr, value, operator, label) \
   Needs to be implemented. Probably the reason the hangar lifts are wonky on coop. Also need to check hangar lifts in coop+anti mode
**  TODO src/include/commands.h|3462 col 1| #define if_chr_knockedout(chr, label) \
   NTD for Team Missions. Will need player character support for Friends of Joanna campaign.
   #+begin_src c
...
**  TODO src/include/commands.h|3514 col 1| #define if_chr_looking_at_object(chr, object, label) \
   NTD for Team Missions. Only used in training (ie =CHR_BOND= only.) For campaign mode, will need to handle this to get 4player hubworld to work.
**  WONTDO src/include/commands.h|3565 col 1| #define if_chr_soundtimer(value, operator, label) \
   NTD. Only used for NPCs.
**  WONTDO src/include/commands.h|3795 col 1| #define if_chr_not_talking(chr, label) \
   NTD. Only used for NPCs.
** TODO src/include/commands.h|4337 col 1| #define if_chr_same_floor_distance_to_pad_lt(chr, distance, pad, label) \
   Low priority. Scripts work well when used in a tight bond / coop loop.
